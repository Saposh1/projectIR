...
#     listi=dict(bm25.bm25_search(query,bodyPath,100))
#     listi1=binarySearch1(query,indexTitle,titlePath)
#     for tup in listi1:
#         if tup[0] in listi:
#             if len(query)>3:
#                 listi[tup[0]]+=tup[1]+ math.log(float(page_rank[str(tup[0])]))
#             else:
#                 listi[tup[0]]+=tup[1]*3+ math.log(float(page_rank[str(tup[0])]))
#         else:
#           listi[tup[0]]=tup[1]*9 + math.log(float(page_rank[str(tup[0])]))+ math.log(float(page_rank[str(tup[0])]))
#     listi= sorted([(doc_id,score) for doc_id, score in listi.items()],key=lambda x:x[1],reverse=True)[:10]
#     return [(tup[0],docs_titles[tup[0]]) for tup in listi]
def best_search(query,indexTitle,body_index,titlePath,bodyPath):
    query=queryRepresent1(query)
    bm25=BM25_from_index(body_index,bodyPath)
    listi=dict(bm25.bm25_search(query,bodyPath,100))
    ...
    for tup in listi1:
        if tup[0] in listi:
            ...
            listi[tup[0]]=tup[1]*9+ math.log(float(page_rank[str(tup[0])]))
    listi= sorted([(doc_id,score) for doc_id, score in listi.items()],key=lambda x:x[1],reverse=True)[:10]
    return [(tup[0],docs_titles[tup[0]]) for tup in listi]
